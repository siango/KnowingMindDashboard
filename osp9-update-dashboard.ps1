...
param(
  ...
  # ถ้า true จะลบไฟล์เก่าทั้งปลายทางก่อนคัดลอก (เฉพาะกรณีเผยแพร่จากโฟลเดอร์)
  [bool]  $CleanDest      = $true
)
...
if ($DestSubfolder -and $ContentDir) { $pagesUrl = $pagesUrl.TrimEnd('/') + "/$DestSubfolder/" }

# เตรียม workspace ชั่วคราว
...
# โหมดเผยแพร่จากโฟลเดอร์ (ถ้ามี ContentDir)
if ($ContentDir) {
  ...
  if ($CleanDest -and (Test-Path $dest)) {
    Info "ลบไฟล์เก่าที่ปลายทาง: $dest"
  ...
  # ใส่ cache-bust marker เพื่อบังคับอัปเดต
  ...
  Ok "เผยแพร่ไฟล์แล้ว → $pagesUrl"
  exit 0
}

# โหมด Cache-Bust (ไม่มี ContentDir)
$marker = ".cache-bust-$ts"
Set-Content -Path (Join-Path $PWD $marker) -Value $ts -Encoding UTF8
git add $marker
git -c user.email="bot@kms" -c user.name="KMSBot" commit -m "cache-bust $ts" | Out-Null
git push origin $Branch
Ok "Cache-Bust เสร็จ • หน้าเว็บจะรีเฟรช cache"
