# === OSPW9: One-Shot Build & Deploy KnowingMindDashboard ===
$ErrorActionPreference = "Stop"

# Path repo
$repo = "C:\AndroidProjects\KnowingMindSuite\KnowingMindDashboard"
Set-Location $repo
...
<body>
  ...
  <main id="app" class="container">Loading...</main>
  <footer class="foot">
    Build: {0} — Public: Landing/Overview/Analytics • Private: Logs/Error/Secrets
  </footer>
</body>
</html>
'@ -f (Get-Date -Format yyyyMMddHHmmss) | Out-File "$repo\index.html" -Encoding UTF8

# --- style.css ---
@'
body{margin:0;font-family:system-ui,Arial,sans-serif;background:#f7f5f0;color:#222}
.topbar{background:#2D2A32;color:#fff;padding:10px 16px;display:flex;align-items:center}
.brand{font-weight:bold;margin-right:20px}
.tabs a{color:#fff;margin-right:12px;text-decoration:none}
.tabs a.active{border-bottom:2px solid #F2C94C}
.container{padding:16px}
.card{background:#fff;border-radius:12px;padding:16px;margin:8px;box-shadow:0 2px 6px rgba(0,0,0,.1)}
.row{display:flex;flex-wrap:wrap;gap:12px}
.kv{display:flex;justify-content:space-between;margin:4px 0}
.progress{background:#eee;border-radius:8px;overflow:hidden;height:12px}
.progress i{display:block;background:#80C2FF;height:100%}
.svgchart{width:100%;max-width:600px;height:auto}
.foot{padding:10px 16px;color:#666;font-size:12px}
.ok{color:green}
'@ | Out-File "$repo\style.css" -Encoding UTF8
...
'@ | Out-File "$repo\app.js" -Encoding UTF8

# --- data.json (mock) ---
@'
{
  "projects":[{"name":"KMA","progress":60,"value_m":120},{"name":"ArunRoo","progress":50,"value_m":80}],
  "analytics":{"completion":55,"retention7":22,"dau":120,"wau":300},
  "checklist_today":[{"task":"Deploy","status":"done"},{"task":"Backup","status":"pending"}]
}
'@ | Out-File "$repo\data.json" -Encoding UTF8

# --- Git push ---
git add .
git commit -m "OSPW9 auto build $(Get-Date -Format u)" --allow-empty
git push origin gh-pages

Write-Host "`n[✓] OSPW9 finished. Reload https://knowingmindproject.website" -ForegroundColor Green
